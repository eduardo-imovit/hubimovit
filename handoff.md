...
- Edge Function: `nova_proposta` exige gestao/adm/corretor e grava `criado_por`. **Deploy pendente (v14).**
- **Ordem obrigatória para ir ao ar:** 1) aplicar a migration → 2) deploy da função (precisa da coluna `criado_por`) → 3) push do frontend (`usePerfil` lê colunas novas; sem a migration, o perfil volta vazio e somem os menus).
